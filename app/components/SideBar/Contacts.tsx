...
function Contacts(data: {
  ...
  const { setSelectChats}= context_val()
  const [down, setDown] = useState(false);
  return (
    <div
    onClick={()=>setSelectChats(true)}
      onMouseOver={() => setDown(true)}
      onMouseOut={() => setDown(false)}
      className="w-full  flex justify-start content-center items-center hover:bg-slate-100 hover:bg-opacity-20 h-[80px] "
    >
      <div className="avatar avatar-online">
        <div className="w-16 h-16 rounded-full">
          <Image height={100} width={100} src={data.pfp} alt="_" />
        </div>
      </div>
