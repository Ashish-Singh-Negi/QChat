import { Three_dot } from "@/app/icons";
...
function Story() {
  const { story, setStorySetting } = context_val();
    ...
    >
      <div>
        <div className="flex w-full px-2 items-center  justify-between text-quick-600 text-2xl h-[60px]">
          {" "}
          Status <Three_dot onClick={() => setList(!list)} />
          {list && (
            <div
              onClick={() => setStorySetting(true)}
              className="absolute top-3 right-9 list-border list-none text-sm text-quick-400 hover:bg-slate-100 hover:bg-opacity-35 p-2 rounded-md hover:txt-color-lvl-1"
            >
        ...
        <div
          ...
          className="flex  items-center h-[80px] mx-1 "
        >
          {" "}
      ...
      <div onClick={() => setList(false)}>
        <h2 className="text-quick-600  border-b-2 border-slate-400 border-opacity-20 pb-3  pl-5 mb-3 txt-color-lvl-3">
          VIEWED
        </h2>
